//
//  OfferedRideDetailsVC2.swift
//  ClassyDrives
//
//  Created by Chandan on 03/05/20.
...
import GoogleMaps
class OfferedRideDetailsVC2: BaseVC {
    ...
    var rideDetailDict : RideDetail2?
    var bookid = String()
    var islocal = ""
    var rideId = String()
    var selectIndex = Int()
//    var bookDetail : RideDetail?
    ...
    @IBAction func getDirection(_ sender: Any) {
        ...
        let lat = Double(rideDetailDict?.rideFromLat ?? "")
        let long = Double(rideDetailDict?.rideFromLong ?? "")
        
        let lat2 = Double( rideDetailDict?.rideFromLat2 ?? "")
        let long2 = Double(rideDetailDict?.rideFromLong2  ?? "" )
       
        let lat3 = Double(rideDetailDict?.rideFromLat3 ?? "")
        let long3 = Double(rideDetailDict?.rideFromLong3 ?? "")
       
        let lat4 = Double(rideDetailDict?.rideToLat  ?? "")
        let long4 = Double(rideDetailDict?.rideToLong  ?? "")
    ...
    func putData(){
       mFirstLocationLbl.text = rideDetailDict?.rideFromAddress ?? ""
       mUserLbl2.text = rideDetailDict?.bio ?? ""
       mSecLoctionLbl.text = rideDetailDict?.rideToAddress ?? ""
       mPriceLbl.text = "$ \(rideDetailDict?.bookAmount ?? "")"
//        if (self.isReceived == false)
//                           {
       // self.mCarNameLbl.text = rideDetailDict?.car_name ?? ""
//        }
        let rideDate = rideDetailDict?.rideDate
        mDateLbl.text = convertDateFormater(rideDate ?? "")
        let rideTime = rideDetailDict?.rideTime
        mTimeLbl.text = timeConversion12(time24: rideTime ?? "")
        if (isReceived == true)
        {
            mSeatLbl.text = "\(rideDetailDict?.bookSeat ?? "") Seat Booked"
            amountLbl.text = "Total amount"
        }
        else
        {
        mSeatLbl.text = "\(rideDetailDict?.rideSeat ?? "0") Seat Available"
        }
//            mTableVw.reloadData()
     //   mCnstntProfileVwHgt.constant = 0
        ...
        if rideDetailDict?.detour == "1"{
            ...
            mDetourLbl.text = "Detour: \(rideDetailDict?.detourMiles ?? "") miles"
            mDetourVw.isHidden = false
            view.layoutIfNeeded()
        }
        if rideDetailDict?.isAutoapproval == "1"{
            mCstntAutoHgt.constant = 60
            mAutoVw.isHidden = false
             view.layoutIfNeeded()
        }
        
        if rideDetailDict?.rideFromAddress2 != ""{
            mLoctionLbl2.text = rideDetailDict?.rideFromAddress2 ?? ""
        ...
        if rideDetailDict?.rideFromAddress3 != ""{
            mLocationLbl3.text = rideDetailDict?.rideFromAddress3 ?? ""
...
//            mCarNameLbl.text =      UserVM.sheard.bookRideDetails[0].ride.car_model
            ...
            if rideDetailDict?.isAutoapproval == "1"{
                ...
                if isFromView == true{
                    ...
                    if self.rideDetailDict?.isAutoapproval == "1"{
                        mCstntAutoHgt.constant = 60
                        mAutoVw.isHidden = false
                        view.layoutIfNeeded()
                    }
                    
                    if self.rideDetailDict?.rideFromAddress2 != ""{
                        mLoctionLbl2.text = rideDetailDict?.rideFromAddress2 ?? ""
                        stop1Location.isHidden = false
                    }else{
                      stop1Location.isHidden = true
                    ...
                    if self.rideDetailDict?.rideFromAddress3 != ""{
                        mLocationLbl3.text = rideDetailDict?.rideFromAddress3 ?? ""
                      stop2Location.isHidden = false
...
//            }
    ...
    @IBAction func mViewProfileBtnAct(_ sender: Any) {
       ...
       {
        destinationvc.userId = rideDetailDict?.rideUser ?? ""
...
extension OfferedRideDetailsVC2: UITableViewDelegate,UITableViewDataSource{
...
//            cell.requestResponseLbl.textColor = .red
             ...
             if (bookStatus != "4" && self.rideDetailDict?.rideStatus == "1" && isReceived == true)
                  {
                      mBookingReqLbl.isHidden = true
                ...
                else
             {
                ...
                if (bookStatus !=  "4" && isReceived == false && self.rideDetailDict?.rideStatus == "1"  )
                {
                    cell.acceptBtn.isHidden = false
                    cell.rejactBtn.isHidden = false

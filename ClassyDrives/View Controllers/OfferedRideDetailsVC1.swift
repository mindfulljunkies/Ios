//
//  OfferedRideDetailsVC1.swift
//  ClassyDrives
//
//  Created by Chandan on 22/04/20.
...
import GoogleMaps
class OfferedRideDetailsVC1: BaseVC {
...
extension OfferedRideDetailsVC1: UITableViewDelegate,UITableViewDataSource{
...
//            cell.requestResponseLbl.textColor = .red
             ...
             if (bookStatus == "5" && self.rideDetailDict?.ride_status == "1" && isReceived == true)
                  {
                      mBookingReqLbl.isHidden = true
                ...
                else
             {
                ...
                if (bookStatus ==  "5" && isReceived == false && self.rideDetailDict?.ride_status == "1"  )
                {
                    cell.acceptBtn.isHidden = false
                    cell.rejactBtn.isHidden = false
                    cell.rejactBtn.isUserInteractionEnabled = false
                ...
                if bookStatus  ==  "4" && isReceived == false{
                            
                            cell.acceptBtn.isHidden = true
                            cell.rejactBtn.isHidden = false
                            cell.rejactBtn.isUserInteractionEnabled = false
                            cell.rejactBtn.setTitle("Ride Completed", for: .normal)
                    cancelBtn.isHidden = true
                //              cell.requestResponseLbl.isHidden = false
                //              cell.requestResponseLbl.text = "Rejected"
                //            cell.requestResponseLbl.textColor = .red
        ...
        UserVM.sheard.cancelRide(user_id: userID, ride_id: rideId, book_id: bookid, action: actionType, book_reason_text: "", comment: "", currentTime: "") { (success, message, error) in
            if error == nil{
                ...
                if success{
                    ...
                    if actionType == "3"{
                        UserVM.sheard.bookRideDetails[0].book[self.selectIndex].book_status = "3"
                        self.mTableVw.reloadData()
                    }else if actionType == "1"{
                        UserVM.sheard.bookRideDetails[0].book[self.selectIndex].book_status = "1"
                        self.mTableVw.reloadData()
                    }
